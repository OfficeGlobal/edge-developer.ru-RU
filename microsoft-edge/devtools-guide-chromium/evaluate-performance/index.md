...
> [!NOTE]
> Чтобы получить сведения о том, как ускорить загрузку страниц, перейдите к разделу [Оптимизация веб-сайта][DevtoolsSpeedGetStarted].  
...
1.  Выберите `Control` + `Shift` + `I` \ (Windows, Linux \) или `Command` + `Option` + `I` \ (macOS \), чтобы открыть DevTools.  
...
1.  Для использования **ЦП**выберите пункт **замедление 4X**.  DevTools загружает ЦП так, чтобы он был 4 медленнее, чем обычно.  
